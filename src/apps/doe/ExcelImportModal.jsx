    ...
    <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between z-10">
          <h2 className="text-base font-semibold text-gray-900">{t("doe.import.title")}</h2>
          <button onClick={onClose} className="size-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100">
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          <div
            ...
            className={`rounded-xl border-2 border-dashed p-4 text-center transition-colors ${excelDragOver ? "border-amber-400 bg-amber-50" : "border-gray-200"}`}
          >
            <p className="text-sm font-medium text-gray-700 mb-1">{t("doe.import.drop")}</p>
          ...
          {excelImportError && (
            <div className="rounded-lg bg-red-50 border border-red-200 px-3 py-2">
              <p className="text-xs font-medium text-red-600 mb-0.5">Erreur d'import</p>
              <p className="text-xs text-red-500">{excelImportError}</p>
            </div>
          )}
          ...
          <div className="rounded-xl bg-gray-50 border border-gray-200 overflow-hidden">
            <div className="border-b border-gray-200 px-3 pt-3 pb-0">
              <p className="text-[11px] font-semibold text-gray-500 mb-2">
                Structure du fichier Excel — <span className="text-amber-600">5 feuilles</span>
              </p>
            ...
            {xlsTab === 'info' && (
              <div className="p-3 space-y-2 text-[11px]">
                <p className="font-semibold text-gray-700">Le fichier Excel doit contenir 5 feuilles dans cet ordre :</p>
                <div className="space-y-1.5">
                  {[
                    ...
                    <div key={label} className="flex items-start gap-2">
                      <span className={`shrink-0 text-[10px] font-semibold text-${c}-600 bg-${c}-50 rounded px-1.5 py-0.5 w-28 text-center`}>{label}</span>
                ...
                <table className="text-[10px] font-mono border-collapse w-full">
                  <thead><tr className="bg-gray-200">
                    {["Champ","Valeur","Description"].map(h => (
                      <th key={h} className="border border-gray-300 px-1.5 py-1 text-left text-gray-600 font-semibold">{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {[
                      ...
                      <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                        {row.map((cell, j) => (
                          <td key={j} className="border border-gray-200 px-1.5 py-1 text-gray-600">{cell}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
            ...
            {xlsTab === 'matrice' && (
              <div className="p-3">
                <p className="text-[10px] text-gray-400 mb-2">
                  En-têtes : <code className="bg-gray-200 px-1 rounded">Essai</code> puis{" "}
                  <code className="bg-gray-200 px-1 rounded">X1_niveau</code>…{" "}
                  puis <code className="bg-gray-200 px-1 rounded">Y1</code>…{" "}
                  Valeurs −1, 0 ou +1 uniquement pour les niveaux.
                </p>
                <table className="text-[10px] font-mono border-collapse w-full">
                  <thead><tr className="bg-gray-200">
                    {["Essai","X1_niveau","X2_niveau","Y1","Y2"].map(h => (
                      <th key={h} className={`border border-gray-300 px-1.5 py-1 text-left font-semibold ${
                        h.endsWith('_niveau') ? 'text-indigo-600'
                        : h.startsWith('Y') ? 'text-amber-600'
                        : 'text-gray-600'
                      }`}>{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {[[1,-1,-1,"",""],[2,1,-1,"68.2",""],[3,-1,1,"",""],[4,1,1,"71.5",""]].map((row, i) => (
                      <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                        {row.map((cell, j) => (
                          <td key={j} className={`border border-gray-200 px-1.5 py-1 ${
                            cell === "" ? "text-gray-300" : "text-gray-600"
                          } ${j === 3 && cell ? "text-amber-600 font-semibold" : ""}`}>
                            {cell}
                          </td>
                        ))}
                      </tr>
