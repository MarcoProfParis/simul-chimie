    ...
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      {part === 0 && (
        <div className="min-h-[80vh] flex flex-col">
          ...
          <div className="text-center mb-8 mt-4">
            <h1 className="text-2xl font-semibold text-gray-900 mb-2">{t("doe.title")}</h1>
            <p className="text-sm text-gray-500">BTS Métiers de la Chimie — Choisissez comment démarrer</p>
          </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            {/* Actions principales */}
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-400 mb-3">Créer ou importer</p>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {/* Nouveau plan */}
                <button onClick={() => setWelcomeModal('new')}
                  className="flex flex-col items-center gap-3 rounded-xl border-2 border-dashed border-indigo-300 bg-indigo-50 p-6 hover:border-indigo-500 hover:bg-indigo-100 transition-all text-center group">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-indigo-700">Nouveau plan</p>
                    <p className="text-xs text-indigo-500 mt-0.5">Définir les facteurs</p>
                  </div>
                </button>

                {/* Charger JSON */}
                <button onClick={() => { setJsonPasteText(''); setJsonImportError(null); setWelcomeModal('json'); }}
                  className="flex flex-col items-center gap-3 rounded-xl border-2 border-dashed border-emerald-300 bg-emerald-50 p-6 hover:border-emerald-500 hover:bg-emerald-100 transition-all text-center group">
                  <div className="size-12 rounded-xl bg-emerald-100 flex items-center justify-center group-hover:bg-emerald-200 transition-colors">
                    <BookOpenIcon className="size-6 text-emerald-600" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-emerald-700">Importer JSON</p>
                    <p className="text-xs text-emerald-500 mt-0.5">Fichier ou coller le texte</p>
                  </div>
                </button>

                {/* Excel/CSV */}
                <button onClick={() => setWelcomeModal('excel')}
                  className="flex flex-col items-center gap-3 rounded-xl border-2 border-dashed border-amber-300 bg-amber-50 p-6 hover:border-amber-500 hover:bg-amber-100 transition-all text-center group">
                  <div className="size-12 rounded-xl bg-amber-100 flex items-center justify-center group-hover:bg-amber-200 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="size-6 text-amber-600">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
                    </svg>
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-amber-700">Importer Excel / CSV</p>
                    <p className="text-xs text-amber-500 mt-0.5">Excel, CSV ou modèle</p>
                  </div>
                </button>
              </div>
            </div>
            ...
            {["débutant", "intermédiaire", "avancé"].map(diff => {
              ...
              const diffBg = {
                "débutant": "border-emerald-200 hover:border-emerald-400 hover:bg-emerald-50",
                "intermédiaire": "border-amber-200 hover:border-amber-400 hover:bg-amber-50",
                "avancé": "border-rose-200 hover:border-rose-400 hover:bg-rose-50",
              };
              return (
                <div key={diff}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {diffExs.map(ex => (
                      <button key={ex.file} onClick={async () => {
                        ...
                        className={`flex flex-col items-start gap-2 rounded-xl border bg-white p-4 transition-all text-left ${diffBg[diff]}`}>
                        <div className="flex items-start justify-between w-full gap-2">
                          <p className="text-xs font-semibold text-gray-900 leading-tight">{ex.title}</p>
                          {ex.real_data && (
                            <span className="shrink-0 text-[9px] font-semibold bg-amber-100 text-amber-700 rounded-full px-1.5 py-0.5">réel</span>
                          )}
                          {ex.imported && (
                            <span className="shrink-0 text-[9px] font-semibold bg-indigo-100 text-indigo-700 rounded-full px-1.5 py-0.5">importé</span>
                          )}
                        </div>
                        <p className="text-[11px] text-gray-500 leading-relaxed">{ex.context}</p>
                      </button>
                    ))}
                  </div>
                </div>
          ...
          {/* Modal Nouveau plan */}
          {welcomeModal === 'new' && (
            <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={() => setWelcomeModal(null)}>
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
                <h2 className="text-base font-semibold text-gray-900 mb-4">Nouveau plan d'expériences</h2>
                <div className="space-y-4 mb-6">
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1.5">Nombre de facteurs</label>
                    <div className="flex items-center gap-3">
                      <input type="range" min="2" max="6" value={newNbFactors} onChange={e => setNewNbFactors(+e.target.value)} className="flex-1" />
                      <span className="w-8 text-center font-semibold font-mono text-indigo-600 text-lg">{newNbFactors}</span>
                    </div>
                    <p className="text-[10px] text-gray-400 mt-1">→ {Math.pow(2, newNbFactors)} essais minimum (plan 2<sup>{newNbFactors}</sup>)</p>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1.5">Nombre de réponses mesurées</label>
                    <div className="flex items-center gap-3">
                      <input type="range" min="1" max="4" value={newNbResponses} onChange={e => setNewNbResponses(+e.target.value)} className="flex-1" />
                      <span className="w-8 text-center font-semibold font-mono text-indigo-600 text-lg">{newNbResponses}</span>
                    </div>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setWelcomeModal(null)} className="flex-1 rounded-lg border border-gray-200 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 transition-colors">{t("common.cancel")}</button>
          ...
          {welcomeModal === 'json' && (
            <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={() => setWelcomeModal(null)}>
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
                <h2 className="text-base font-semibold text-gray-900 mb-4">Importer un fichier JSON</h2>
                {/* Zone drag & drop */}
                <div
                  ...
                  className={`rounded-xl border-2 border-dashed p-4 text-center mb-3 transition-colors ${jsonDragOver ? "border-indigo-400 bg-indigo-50" : "border-gray-200"}`}>
                  <p className="text-xs text-gray-500">Glissez un fichier .json ici</p>
                ...
                <textarea
                  ...
                  rows={5}
                  className="w-full rounded-lg border border-gray-200 bg-transparent px-3 py-2 text-xs font-mono text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                />
                {jsonImportError && (
                  <p className="mt-2 text-xs text-red-500">{jsonImportError}</p>
                )}
              ...
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div>
                    ...
                    <h2 className="text-base font-semibold text-gray-900">{exampleEditData.title}</h2>
                  </div>
                  {exampleEditData.ex?.real_data && <span className="shrink-0 text-[10px] font-semibold bg-amber-100 text-amber-700 rounded-full px-2 py-0.5">données réelles</span>}
                </div>
                <p className="text-xs text-gray-500 mb-4">{exampleEditData.context}</p>
                    ...
                    <div className="space-y-1">
                      {exampleEditData.data.responses?.map(r => (
                        <div key={r.id} className="flex items-center gap-2 text-xs">
                          ...
                          <span className="font-medium text-gray-700">{r.name}{r.unit?' ('+r.unit+')':''}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                >
                  <div className="flex h-full flex-col bg-white shadow-xl overflow-y-auto">
                    <div className="flex items-center justify-between px-4 py-4 border-b border-gray-200">
                      ...
                      <button onClick={() => setSidebarOpen(false)} className="rounded-md p-1 text-gray-400 hover:text-gray-600 transition-colors">
                    ...
                    <div className="px-4 pt-4 pb-0">
                      <label className="w-full flex items-center justify-center gap-2 rounded-lg border border-gray-200 px-4 py-2.5 text-sm font-medium text-gray-600 hover:border-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors cursor-pointer">
                      ...
                      {importError && (
                        <div className="mt-2 rounded-lg bg-red-50 border border-red-200 px-3 py-2">
                          <p className="text-xs font-medium text-red-600 mb-0.5">Format invalide</p>
                          <p className="text-[11px] text-red-500 leading-relaxed">{importError}</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="px-4 pt-4 pb-2">
                      <button onClick={resetToNew}
                        className="w-full flex items-center justify-center gap-2 rounded-lg border-2 border-dashed border-gray-300 px-4 py-3 text-sm font-medium text-gray-600 hover:border-gray-400 hover:text-gray-800 transition-colors">
                    ...
                    <div className="px-4 py-3">
                      <p className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 mb-3">Exemples</p>
                      <div className="flex flex-col gap-2">
                        {EXAMPLE_FILES.map((ex) => (
                          <div key={ex.file} className={`rounded-lg border transition-all ${
                            ...
                            <button onClick={() => loadExample(ex)} className="w-full text-left px-3 pt-2.5 pb-2 hover:bg-gray-50 rounded-t-lg transition-colors">
                              <div className="flex items-start justify-between gap-2 mb-1">
                                <span className="text-xs font-medium text-gray-900 leading-tight">{ex.title}</span>
                                {ex.real_data && (
                                  <span className="shrink-0 text-[10px] font-semibold bg-amber-100 text-amber-700 rounded-full px-1.5 py-0.5">réel</span>
                                )}
                              </div>
                              <p className="text-[11px] text-gray-500 mb-1.5">{ex.context}</p>
                    ...
                    {loadError && (
                      <div className="mx-4 mt-2 mb-4 rounded-lg bg-red-50 border border-red-200 px-3 py-2">
                        <p className="text-xs text-red-600 font-medium mb-0.5">Erreur de chargement</p>
                        <p className="text-[11px] text-red-500 break-all">{loadError}</p>
                      </div>
                    )}
                    </div>
                    ...
                    {importedExamples.length > 0 && (
                      <div className="px-4 py-3 border-t border-gray-100">
                        <div className="flex items-center justify-between mb-3">
                          <p className="text-[10px] font-semibold uppercase tracking-widest text-indigo-400">Importés</p>
                          ...
                          {importedExamples.map((ex) => (
                            <div key={ex.file} className={`rounded-lg border transition-all ${
                              ...
                              <button onClick={() => loadExample(ex)} className="w-full text-left px-3 pt-2.5 pb-2 hover:bg-gray-50 rounded-t-lg transition-colors">
                                <div className="flex items-start justify-between gap-2 mb-1">
                                  <span className="text-xs font-medium text-gray-900 leading-tight">{ex.title}</span>
                                  <span className="shrink-0 text-[10px] font-semibold bg-indigo-100 text-indigo-700 rounded-full px-1.5 py-0.5">importé</span>
                                </div>
                                <p className="text-[11px] text-gray-500 mb-1.5">{ex.context}</p>
                                <span className={`inline-block text-[10px] font-semibold rounded-full px-1.5 py-0.5 ${diffBadgeCls[ex.difficulty] || diffBadgeCls["débutant"]}`}>
                                  {ex.difficulty}
                ...
                {status === "complete" ? (
                  <button onClick={() => goTo(s.n)} className="group flex w-full items-center">
                    <span className="flex items-center px-6 py-4 text-sm font-medium">
                      ...
                      <span className="ml-4 text-sm font-medium text-gray-900">{s.l}</span>
                    </span>
                  </button>
                ) : status === "current" ? (
                  <button onClick={() => goTo(s.n)} aria-current="step" className="flex items-center px-6 py-4 text-sm font-medium">
                    ...
                    <span className="ml-4 text-sm font-medium text-indigo-600">{s.l}</span>
                  </button>
                ) : (
                  <button onClick={() => goTo(s.n)} className="group flex items-center">
                    <span className="flex items-center px-6 py-4 text-sm font-medium">
                      <span className="flex size-10 shrink-0 items-center justify-center rounded-full border-2 border-gray-300 group-hover:border-gray-400">
                        <span className="text-gray-500 group-hover:text-gray-900">{s.id}</span>
                      </span>
                      <span className="ml-4 text-sm font-medium text-gray-500 group-hover:text-gray-900">{s.l}</span>
                    </span>
                  </button>
                )}
                {i !== arr.length - 1 && (
      ...
      {part === 1 && (
        <>
          ...
          {editMode && (
            <div className="flex items-center justify-between bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-3 mb-4 gap-3">
              <div className="flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="size-4 text-indigo-500">
                  <path d="M2.695 14.763l-1.262 3.154a.5.5 0 00.65.65l3.155-1.262a4 4 0 001.343-.885L17.5 5.5a2.121 2.121 0 00-3-3L3.58 13.42a4 4 0 00-.885 1.343z" />
                </svg>
                <span className="text-sm font-medium text-indigo-700">Mode édition — {editMeta.title || loadedExampleId}</span>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => { setEditMode(false); }}
                  className="rounded-md border border-indigo-200 px-2.5 py-1 text-xs text-indigo-600 hover:bg-indigo-100 transition-colors">
                  {t("common.cancel")}
                </button>
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <button onClick={() => setShowExportPopup(true)}
              className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors shadow-sm"
          ...
          {showExportPopup && (
            <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={() => setShowExportPopup(false)}>
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-4">
                  <p className="text-sm font-semibold text-gray-900">Sauvegarder le plan</p>
                  <button onClick={() => setShowExportPopup(false)} className="size-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100">
                    <XMarkIcon className="size-5" />
                  </button>
                </div>
                {loadedExampleId && (
                  <p className="text-xs text-gray-400 mb-4 truncate">
                    Plan : <span className="text-gray-600 font-medium">{[...EXAMPLE_FILES, ...importedExamples].find(e => e.file === loadedExampleId)?.title || loadedExampleId}</span>
                  </p>
                )}
                <div className="flex flex-col gap-2">
                  <button onClick={() => { exportJSON(); setShowExportPopup(false); }}
                    className="flex items-center gap-3 rounded-xl border border-gray-200 px-4 py-3 text-sm text-gray-700 hover:border-indigo-300 hover:bg-indigo-50 transition-all">
                    <div className="size-8 rounded-lg bg-indigo-100 flex items-center justify-center">
                      <span className="text-[10px] font-bold text-indigo-600">JSON</span>
                    </div>
                    <div>
                  ...
                  <button onClick={() => { exportCSV(); setShowExportPopup(false); }}
                    className="flex items-center gap-3 rounded-xl border border-gray-200 px-4 py-3 text-sm text-gray-700 hover:border-emerald-300 hover:bg-emerald-50 transition-all">
                    <div className="size-8 rounded-lg bg-emerald-100 flex items-center justify-center">
                      <span className="text-[10px] font-bold text-emerald-600">CSV</span>
                    </div>
                    <div>
                  ...
                  <button onClick={() => { exportXLSX(); setShowExportPopup(false); }}
                    className="flex items-center gap-3 rounded-xl border border-gray-200 px-4 py-3 text-sm text-gray-700 hover:border-amber-300 hover:bg-amber-50 transition-all">
                    <div className="size-8 rounded-lg bg-amber-100 flex items-center justify-center">
                      <span className="text-[10px] font-bold text-amber-600">XLS</span>
                    </div>
                    <div>
          ...
          {showQuitConfirm && (
            <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={() => setShowQuitConfirm(false)}>
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
                ...
                <p className="text-base font-semibold text-gray-900 text-center mb-1">Sauvegarder avant de quitter ?</p>
                ...
                <div className="flex flex-col gap-2 mb-3">
                  <button onClick={() => { exportJSON(); setShowQuitConfirm(false); setPart(0); }}
                    className="flex items-center gap-3 rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-700 hover:border-indigo-300 hover:bg-indigo-50 transition-all">
                    <span className="size-7 rounded-lg bg-indigo-100 flex items-center justify-center text-[9px] font-bold text-indigo-600">JSON</span>
                    Sauvegarder en JSON et quitter
                  </button>
                  <button onClick={() => { exportCSV(); setShowQuitConfirm(false); setPart(0); }}
                    className="flex items-center gap-3 rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-700 hover:border-emerald-300 hover:bg-emerald-50 transition-all">
                    <span className="size-7 rounded-lg bg-emerald-100 flex items-center justify-center text-[9px] font-bold text-emerald-600">CSV</span>
                    Sauvegarder en CSV et quitter
                  </button>
                  <button onClick={() => { exportXLSX(); setShowQuitConfirm(false); setPart(0); }}
                    className="flex items-center gap-3 rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-700 hover:border-amber-300 hover:bg-amber-50 transition-all">
                    <span className="size-7 rounded-lg bg-amber-100 flex items-center justify-center text-[9px] font-bold text-amber-600">XLS</span>
                    Sauvegarder en Excel et quitter
                  </button>
                </div>
                <button onClick={() => { setShowQuitConfirm(false); setPart(0); }}
                  className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-500 hover:bg-gray-50 transition-colors">
          ...
          {editMode && (
            <div className="bg-white border border-indigo-200 rounded-xl p-5 mb-4">
              <p className="text-[11px] font-semibold uppercase tracking-widest text-indigo-400 mb-3">Métadonnées de l'exemple</p>
              <div className="grid grid-cols-1 gap-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">ID fichier</label>
                    <input value={editMeta.id} onChange={e => setEditMeta({ ...editMeta, id: e.target.value })}
                      className="w-full rounded-md border border-gray-200 bg-transparent px-2 py-1.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                  </div>
                  <div>
                    ...
                    <select value={editMeta.difficulty} onChange={e => setEditMeta({ ...editMeta, difficulty: e.target.value })}
                      className="w-full rounded-md border border-gray-200 bg-white px-2 py-1.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <div>
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-sm text-gray-700">Données réelles (badge "réel")</span>
                  </label>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-400 mb-3 flex items-center gap-2">Facteurs <HelpButton topic="facteurs" size="xs" /></p>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    {["ID", "Nom", "Unité", "Type", "Niveau bas", "Niveau haut", ""].map((h, i) => (
                      <th key={i} className="text-left text-[11px] font-medium text-gray-400 pb-2 px-2 whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {factors.map((f, i) => (
                    <tr key={f.id} className="border-b border-gray-50 last:border-0">
                      <td className="px-2 py-1.5">
                        <span className="font-mono text-xs font-semibold text-gray-400">{f.id}</span>
                      </td>
                      <td className="px-2 py-1.5">
                        <input value={f.name} onChange={e => updateFactor(i, "name", e.target.value)}
                          className="w-28 rounded-md border border-gray-200 bg-transparent px-2 py-1 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                      </td>
                      <td className="px-2 py-1.5">
                        <input value={f.unit || ""} placeholder="°C, g…" onChange={e => updateFactor(i, "unit", e.target.value)}
                          className="w-16 rounded-md border border-gray-200 bg-transparent px-2 py-1 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                      </td>
                      <td className="px-2 py-1.5">
                        <select value={String(f.continuous)} onChange={e => updateFactor(i, "continuous", e.target.value === "true")}
                          className="rounded-md border border-gray-200 bg-white px-2 py-1 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                      ...
                      <td className="px-2 py-1.5">
                        {f.continuous
                          ? <input type="number" value={f.low.real} onChange={e => updateFactorLevel(i, "low", e.target.value)}
                              className="w-20 rounded-md border border-gray-200 bg-transparent px-2 py-1 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                          : <input value={f.low.label || ""} placeholder="Label −1" onChange={e => updateFactorLabel(i, "low", e.target.value)}
                              className="w-24 rounded-md border border-gray-200 bg-transparent px-2 py-1 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                        }
                      </td>
                      <td className="px-2 py-1.5">
                        {f.continuous
            ...
            {factors.length < 6
              ? <button onClick={addFactor}
                  className="mt-3 flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-600 border border-dashed border-gray-300 rounded-lg px-3 py-1.5 transition-colors">
          ...
          {/* Réponses */}
          <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-400 mb-3">Réponses</p>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    {["ID", "Nom", "Unité", ""].map((h, i) => (
                      <th key={i} className="text-left text-[11px] font-medium text-gray-400 pb-2 px-2">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={r.id} className="border-b border-gray-50 last:border-0">
                      ...
                      <td className="px-2 py-1.5">
                        <input value={r.name} onChange={e => updateResponse(i, "name", e.target.value)}
                          className="w-36 rounded-md border border-gray-200 bg-transparent px-2 py-1 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                      </td>
                      <td className="px-2 py-1.5">
          ...
          {/* Point central */}
          <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-400 mb-3">Point central</p>
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={centerPoint.present}
                  onChange={e => setCenterPoint({ ...centerPoint, present: e.target.checked })}
                  className="size-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                <span className="text-sm text-gray-700">Inclure un point central</span>
              </label>
              {centerPoint.present && (
                <div className="flex items-center gap-2">
          ...
          <div className="flex items-center justify-between mt-5">
            <p className="text-xs text-gray-400">
              Plan 2<sup>{factors.length}</sup> = <strong className="text-gray-600">{1 << factors.length}</strong> essai(s)
        ...
        <>
          ...
          <Dialog open={addRowLevels !== null} onClose={() => setAddRowLevels(null)} className="relative z-50">
            ...
            <div className="fixed inset-0 z-10 flex items-center justify-center p-4">
              <DialogPanel transition className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl transition-all data-closed:opacity-0 data-closed:scale-95">
                <DialogTitle className="text-sm font-semibold text-gray-900 mb-4">
                ...
                {addRowLevels && factors.map(f => {
                  ...
                  return (
                    <div key={f.id} className="mb-3">
                      <p className="text-xs text-gray-500 mb-1.5">{f.id} — {f.name}</p>
                      <div className="flex gap-2">
                        <button onClick={() => setAddRowLevels({ ...addRowLevels, [f.id]: -1 })}
                          className={`flex-1 rounded-lg border px-2 py-1.5 text-xs font-mono font-medium transition-colors ${sel === -1 ? "bg-red-50 border-red-300 text-red-700" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
                          −1{f.continuous ? ` (${f.low.real}${f.unit ? " " + f.unit : ""})` : (f.low.label ? " " + f.low.label : "")}
                        </button>
                        {f.continuous && (
                          <button onClick={() => setAddRowLevels({ ...addRowLevels, [f.id]: 0 })}
                            className={`flex-1 rounded-lg border px-2 py-1.5 text-xs font-mono font-medium transition-colors ${sel === 0 ? "bg-amber-50 border-amber-300 text-amber-700" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
                            0 ({+((f.low.real + f.high.real) / 2).toFixed(2)}{f.unit ? " " + f.unit : ""})
                          </button>
                        )}
                        <button onClick={() => setAddRowLevels({ ...addRowLevels, [f.id]: 1 })}
                          className={`flex-1 rounded-lg border px-2 py-1.5 text-xs font-mono font-medium transition-colors ${sel === 1 ? "bg-emerald-50 border-emerald-300 text-emerald-700" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
                ...
                <div className="flex justify-end gap-2 mt-5">
                  <button onClick={() => setAddRowLevels(null)}
                    className="rounded-md bg-white border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                    {t("common.cancel")}
                  </button>
          ...
          <Dialog open={showRandomDialog} onClose={setShowRandomDialog} className="relative z-50">
            <DialogBackdrop transition className="fixed inset-0 bg-gray-900/50 transition-opacity data-closed:opacity-0" />
            <div className="fixed inset-0 z-10 flex items-center justify-center p-4">
              <DialogPanel transition className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl transition-all data-closed:opacity-0 data-closed:scale-95">
                <div className="mx-auto mb-4 flex size-12 items-center justify-center rounded-full bg-amber-100">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setShowRandomDialog(false)}
                    className="rounded-md bg-white border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                    {t("common.cancel")}
                  </button>
          ...
          <Dialog open={showRandomDone} onClose={setShowRandomDone} className="relative z-50">
            <DialogBackdrop transition className="fixed inset-0 bg-gray-900/50 transition-opacity data-closed:opacity-0" />
            <div className="fixed inset-0 z-10 flex items-center justify-center p-4">
              <DialogPanel transition className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl text-center transition-all data-closed:opacity-0 data-closed:scale-95">
                ...
                <DialogTitle className="text-base font-semibold text-gray-900 mb-2">Valeurs générées</DialogTitle>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
            <div className="flex items-center justify-between mb-4">
              <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-400">Matrice d'expériences</p>
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-400">{matrix.length} essai(s)</span>
                <button onClick={openAddRow}
                  className="flex items-center gap-1 rounded-md border border-gray-200 px-2.5 py-1 text-xs text-gray-600 hover:bg-gray-50 transition-colors">
            ...
            {hasMissing && (
              <div className="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-4 gap-3">
                <span className="text-xs text-amber-700">{missingRows.length} ligne(s) sans réponse complète.</span>
                <button onClick={() => setShowRandomDialog(true)}
                  className="shrink-0 rounded-md bg-amber-100 border border-amber-300 px-2.5 py-1 text-xs font-medium text-amber-700 hover:bg-amber-200 transition-colors">
              ...
              <table className="w-full text-sm">
                  ...
                  <tr className="border-b border-gray-100">
                    ...
                    {factors.map(f => (
                      <th key={f.id} className="text-left text-[11px] font-medium text-gray-400 pb-2 px-2 whitespace-nowrap">
                        {f.id}<br />
                        <span className="text-[10px] font-normal text-gray-300">{f.name}{f.unit ? ` (${f.unit})` : ""}</span>
                      </th>
                    ))}
                    <th className="w-px bg-gray-100 p-0" />
                    {responses.map(r => (
                      <th key={r.id} className="text-left text-[11px] font-medium text-emerald-600 pb-2 px-2 whitespace-nowrap">
                        {r.id}<br />
                        <span className="text-[10px] font-normal text-emerald-400">{r.name}{r.unit ? ` (${r.unit})` : ""}</span>
                      </th>
                    ))}
                    <th className="w-8" />
                  </tr>
                ...
                <tbody>
                  {matrix.map((row, ri) => {
                    ...
                    return (
                      <tr key={ri} className={`border-b border-gray-50 last:border-0 ${row.center ? "bg-amber-50/40" : isMissing ? "bg-red-50/40" : ""}`}>
                        ...
                        {factors.map(f => {
                          ...
                          const cCls = c === -1 ? "text-red-500" : c === 1 ? "text-emerald-600" : "text-amber-500";
                          if (row.center && !f.continuous) return (
                            <td key={f.id} className="px-2 py-1.5 text-center text-xs text-gray-300">—</td>
                          );
                          return (
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                {f.continuous
                                  ? <input type="number" value={rv} onChange={e => updateCell(ri, f.id, e.target.value)}
                                      className="w-14 rounded border border-transparent bg-transparent px-1 py-0.5 text-xs text-gray-700 hover:border-gray-200 focus:outline-none focus:border-indigo-400 transition-colors" />
                                  : <span className="text-xs text-gray-500">{rv ?? "—"}</span>
                                }
                              </div>
                            </td>
                          );
                        })}
                        <td className="w-px bg-gray-100 p-0" />
                        {responses.map(r => {
                          ...
                          return (
                            <td key={r.id} className="px-2 py-1.5">
                              <input type="number" value={isEmpty ? "" : v} placeholder="—"
                                ...
                                className={`w-16 rounded border bg-transparent px-1 py-0.5 text-xs text-emerald-700 hover:border-gray-200 focus:outline-none focus:border-indigo-400 transition-colors ${isEmpty ? "border-red-300" : "border-transparent"}`} />
                            </td>
                          );
                        })}
                        <td className="px-2 py-1.5">
                          <button onClick={() => removeRun(ri)}
                            className="p-1 rounded text-red-300 hover:text-red-500 hover:bg-red-50 transition-colors">
        ...
        return (
          <>
            ...
            {activeModel && (() => {
              ...
              return (
                <div className={`bg-white ${cardCls} ${col.border} mb-4`}>
                  {/* Header modèle */}
                  <div className="flex items-center justify-between mb-4 gap-3">
                    <input value={activeModel.name} onChange={e => renameModel(activeModel.id, e.target.value)}
                      className="text-sm font-semibold bg-transparent border-b border-transparent hover:border-gray-300 focus:outline-none focus:border-indigo-400 text-gray-900 w-40 transition-colors" />
                    <div className="flex items-center gap-2">
                      {!isDefault && (
                  ...
                  {/* Constante */}
                  <div className="mb-3">
                    <span className="inline-flex items-center rounded-full bg-purple-100 border border-purple-200 px-3 py-1 text-xs font-mono font-semibold text-purple-700 mr-2">α₀</span>
                  ...
                  {orderedKeys.map(order => (
                    <div key={order} className="mb-3">
                      <p className="text-xs text-gray-400 mb-2">{orderLabels[order]}</p>
      ...
      {part === 5 && (() => {
        ...
        return (
          <>
            {/* Rangée : sélecteur réponse (gauche) + bouton PDF (droite) */}
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                {responses.length > 1 && (
                  <>
                    ...
                    <div className="flex gap-2">
                      {responses.map((r, i) => (
                        <button key={r.id} onClick={() => setPart4Response(i)}
                          className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors ${i === part4Response ? "bg-indigo-600 border-indigo-600 text-white" : "border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
            ...
            {part4Tab === "effets_calcul" && (
              <div className={cardSpace}>
                ...
                {models.map((m, mi) => {
                  ...
                  const col = modelColors[mi % modelColors.length];
                  return (
                    <div key={m.id} className={`bg-white ${cardCls} ${col.border}`}>
                      <div className="flex items-center gap-2 mb-4">
            ...
            {part4Tab === "coefficients" && (
              <div className={cardSpace}>
                <div className="flex items-center gap-2 mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">{t("doe.effects")}</h3>
                          ...
                          <table className="w-full text-xs">
                            ...
                            <tbody>
                              {fit.coeffs.map((c, ci) => {
                                ...
                                return (
                                  <tr key={ci} className={`border-b border-gray-50 last:border-0 ${isSignif ? "bg-indigo-50/30" : ""}`}>
                                    <td className="px-2 py-1.5 font-mono text-gray-700">{labels[ci]}</td>
                                    <td className="px-2 py-1.5 text-right font-mono font-semibold text-gray-900">{fmt(c)}</td>
                                    ...
                                    <td className={`px-2 py-1.5 text-right font-mono ${isSignif ? "text-indigo-600 font-semibold" : "text-gray-400"}`}>{fmtP(p)}</td>
            ...
            {/* ── TAB : RÉSIDUS ── */}
              ...
              <div className={cardSpace}>
                <div className="flex items-center gap-2 mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">{t("doe.residuals")}</h3>
                ...
                {(() => {
                  ...
                  return (
                    <div className={`bg-white ${cardCls} ${col.border}`}>
                      ...
                      <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-400 mb-3">Résidus vs Ŷ — cliquer sur un point pour le détail</p>
                      <ResidualPlot
                        yHat={fit.yHat}
                      ...
                      {fit.residuals.length >= 3 && (
                        <div className="mt-5 border-t border-gray-100 pt-4">
                          <div className="flex items-center gap-2 mb-3">
                            <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-400">Q-Q Plot — Normalité des résidus</p>
                          ...
                          <div className="flex flex-col sm:flex-row gap-4 items-start">
                            ...
                            <div className="flex-1 space-y-2">
                              {(() => {
                                ...
                                return (
                                  <>
                                    <div className={`rounded-lg px-3 py-2 text-xs ${isNormal ? "bg-emerald-50 border border-emerald-200" : "bg-amber-50 border border-amber-200"}`}>
                                      <p className={`font-semibold mb-1 ${isNormal ? "text-emerald-700" : "text-amber-700"}`}>
                                        {isNormal ? "✓ Normalité probable" : "△ Normalité questionnable"}
                                      </p>
                                      <p className={isNormal ? "text-emerald-600" : "text-amber-600"}>
                                        {isNormal
                                    ...
                                    <div className="rounded-lg bg-gray-50 border border-gray-100 px-3 py-2 text-xs text-gray-500 space-y-1">
                                      <p className="font-medium text-gray-600">Comment lire :</p>
            ...
            {part4Tab === "anova" && (
              <div className={cardSpace}>
                <div className="flex items-center gap-2 mb-3">
                  <h3 className="text-sm font-semibold text-gray-900">{t("doe.anova")}</h3>
                ...
                {models.map((m, mi) => {
                  ...
                  const verdictCls = verdict === "acceptable" ? "bg-emerald-100 text-emerald-800" : verdict === "à rejeter" ? "bg-red-100 text-red-800" : "bg-amber-100 text-amber-800";
                  return (
                    <div key={m.id} className={`bg-white ${cardCls} ${col.border}`}>
                      <div className="flex items-center gap-2 mb-4">
                        <span className={`size-2.5 rounded-full ${col.dot}`} />
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                        {[
                          ...
                          <div key={stat.label} className="bg-gray-50 rounded-lg p-3 text-center">
                            ...
                            <p className="text-sm font-semibold font-mono text-gray-900">{stat.value}</p>
                          </div>
                        ))}
                      </div>
                      {/* Tableau ANOVA */}
                          ...
                          <tbody>
                            {[
                              ...
                              <tr key={row.label} className="border-b border-gray-50 last:border-0">
                                <td className="px-2 py-1.5 font-medium text-gray-700">{row.label}</td>
                                <td className="px-2 py-1.5 text-right font-mono text-gray-600">{fmt(row.sc, 4)}</td>
                                <td className="px-2 py-1.5 text-right font-mono text-gray-600">{row.df}</td>
                                <td className="px-2 py-1.5 text-right font-mono text-gray-600">{row.cm !== null ? fmt(row.cm, 4) : "—"}</td>
                                <td className="px-2 py-1.5 text-right font-mono text-gray-600">{row.f !== null ? fmt(row.f, 3) : "—"}</td>
                                <td className={`px-2 py-1.5 text-right font-mono ${row.p !== null && row.p < 0.05 ? "text-indigo-600 font-semibold" : "text-gray-400"}`}>{row.p !== null ? fmtP(row.p) : "—"}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                      ...
                      {/* Verdict détaillé */}
                      <div className={`rounded-lg p-3 ${verdict === "acceptable" ? "bg-emerald-50 border border-emerald-200" : verdict === "à rejeter" ? "bg-red-50 border border-red-200" : "bg-amber-50 border border-amber-200"}`}>
                        <div className="flex items-center justify-between mb-2">
                          <p className="text-xs font-medium text-gray-700">Analyse de validation</p>
                          <button
                            type="button"
                        ...
                        <ul className="text-xs space-y-1">
                          <li className={`flex items-start gap-1.5 ${R2ok ? "text-emerald-700" : "text-amber-700"}`}>
                            <span>{R2ok ? "✓" : "△"}</span>
                            <span>R² ajusté = {fmt(fit.R2adj, 4)} {R2ok ? "(bon ajustement ≥ 0.8)" : "(ajustement insuffisant < 0.8)"}</span>
                          </li>
                          <li className={`flex items-start gap-1.5 ${modelOK ? "text-emerald-700" : "text-red-700"}`}>
                            <span>{modelOK ? "✓" : "✗"}</span>
                        ...
                        {verdict !== "acceptable" && (
                          <div className="mt-2 pt-2 border-t border-red-200 flex items-center justify-between">
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 text-[10px] font-medium text-white bg-red-500 hover:bg-red-600 px-2.5 py-1 rounded-md transition-colors"
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="size-3">
            ...
            {part4Tab === "effets" && (
              <div className={cardSpace}>
                <div className="flex items-center gap-2 mb-4">
                  <h3 className="text-sm font-semibold text-gray-900">{t("doe.pareto")}</h3>
                ...
                {models.map((m, mi) => {
                  ...
                  return (
                    <div key={m.id} className={`bg-white ${cardCls} ${col.border}`}>
                      <div className="flex items-center gap-2 mb-4">
                        <span className={`size-2.5 rounded-full ${col.dot}`} />
                        <h3 className="text-sm font-semibold text-gray-900">{m.name} — {t("doe.pareto")}</h3>
                      </div>
                      {/* ── Diagramme de Pareto ── */}
                      {(() => {
                        ...
                        return (
                          <div className="space-y-1">
                            {/* Bandeau "tout gris" */}
                            {allNS && (
                              <div className="flex items-start gap-3 rounded-xl bg-amber-50 border border-amber-300 px-4 py-3 mb-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex flex-wrap gap-2 mt-2">
                                    {fit.dfE < 2 && (
                                      <span className="inline-flex items-center gap-1 text-[10px] bg-amber-100 text-amber-800 px-2 py-0.5 rounded-full">
                              ...
                              return (
                                <div key={ef.term} className="flex items-center gap-2">
                                  ...
                                  <span className={`text-[10px] font-mono w-14 text-right shrink-0 ${
                                    isSignif ? "text-gray-500" : "text-gray-300"
                                  }`}>
                                    {ef.absCoeff.toFixed(3)}
                                  </span>
                                  {ef.p !== null && (
                                    <span className={`text-[10px] font-mono w-12 text-right shrink-0 ${
                                      isSignif ? "text-indigo-600 font-semibold" : "text-gray-400"
                                    }`}>
                                      {pFmt}
                                    </span>
                                  )}
                                  {isNS && (
                                    <button
                                      type="button"
                                      title={helpMsg}
                                      className="shrink-0 size-4 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 hover:bg-amber-100 hover:text-amber-700 transition-colors"
                                        ...
                                        <button
                                          ...
                                          className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[11px] font-mono transition-colors ${
                                            disabled
                                              ...
                                              : "border-gray-200 text-gray-500 hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-600 cursor-pointer"
                                          }`}
                                        >
                                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="size-3">
            ...
            {part4Tab === "isoresponse" && (
              <div className={cardSpace}>
                <div className="flex items-center gap-2 mb-4">
                  <h3 className="text-sm font-semibold text-gray-900">{t("doe.isoresponse")}</h3>
        ...
        return (
          <div className="fixed inset-0 z-50 bg-black/30 flex items-end sm:items-center justify-center p-4"
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto"
              ...
              <div className="sticky top-0 bg-indigo-50 border-b border-indigo-100 px-5 py-4 flex items-center gap-3">
                <div className="size-9 rounded-lg bg-indigo-100 flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="size-5 text-indigo-600">
                ...
                <div className="flex-1">
                  ...
                  <h2 className="text-sm font-semibold text-gray-900">Comment sont calculés ces indicateurs ?</h2>
                </div>
              ...
              <div className="px-5 py-4 space-y-4 text-xs">
                <div className="rounded-xl border border-blue-200 overflow-hidden">
                  <div className="bg-blue-50 px-4 py-2 font-semibold text-blue-800 text-sm">
                    R² — Coefficient de détermination
                  </div>
                  <div className="px-4 py-3 space-y-2">
                    <div className="bg-gray-50 rounded-lg p-3 font-mono text-indigo-700">
                ...
                <div className="rounded-xl border border-blue-200 overflow-hidden">
                  ...
                  <div className="px-4 py-3 space-y-2">
                    <div className="bg-gray-50 rounded-lg p-3 font-mono text-indigo-700 text-[11px]">
              ...
              <div className="px-5 py-3 border-t border-gray-100 bg-gray-50 text-center text-[10px] text-gray-400">
      ...
      {improvementHelpFit && (() => {
        ...
        return (
          <div className="fixed inset-0 z-50 bg-black/30 flex items-end sm:items-center justify-center p-4"
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto"
                 onClick={e => e.stopPropagation()}>
              <div className="sticky top-0 bg-red-50 border-b border-red-100 px-5 py-4 flex items-center gap-3">
                <div className="size-9 rounded-lg bg-red-100 flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="size-5 text-red-600">
                ...
                <div className="flex-1">
                  ...
                  <h2 className="text-sm font-semibold text-gray-900">Comment améliorer ce modèle ?</h2>
                </div>
              ...
              <div className="px-5 py-4 space-y-3 text-xs">
                <div className="rounded-lg bg-red-50 border border-red-200 px-4 py-3">
                  <p className="font-semibold text-red-700 mb-2">Problèmes détectés :</p>
                ...
                <p className="font-semibold text-gray-700">Pistes d'amélioration :</p>

                {dfTooLow && (
                  <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
                    <p className="font-semibold text-amber-700 mb-1">1. Réduire le nombre de termes</p>
                ...
                {r2TooLow && !dfTooLow && (
                  <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
                    <p className="font-semibold text-blue-700 mb-1">3. Ajouter des termes de courbure</p>
                ...
                {!dfTooLow && !r2TooLow && probTooHigh && (
                  <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
                    <p className="font-semibold text-blue-700 mb-1">3. Vérifier les données</p>
                    <p className="text-blue-600">
                      Avec R² ajusté = {(+fit.R2adj).toFixed(3)} et Prob &gt; F = {fp(fit.pF)}, la variabilité
                      expérimentale est peut-être trop grande. Vérifier :
                    </p>
                    <ul className="mt-1 space-y-0.5 text-blue-600 ml-3">
                ...
                <div className="rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-3">
                  <p className="font-semibold text-indigo-700 mb-1">Rappel : règle de décision</p>
        ...
        return (
          <div className="fixed inset-0 z-50 bg-black/30 flex items-end sm:items-center justify-center p-4"
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[80vh] overflow-y-auto"
                 onClick={e => e.stopPropagation()}>
              <div className="sticky top-0 bg-gray-50 border-b border-gray-100 px-5 py-4 flex items-center gap-3">
                <div className="size-9 rounded-lg bg-gray-200 flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="size-5 text-gray-500">
                ...
                <div className="flex-1">
                  ...
                  <h2 className="text-sm font-semibold text-gray-900 font-mono">{label}</h2>
                </div>
              ...
              <div className="px-5 py-4 space-y-3 text-xs">
                <div className="rounded-lg bg-gray-50 border border-gray-200 px-4 py-3">
                  <p className="font-semibold text-gray-700 mb-2">Résultat du test de Student</p>
                  <div className="space-y-1 font-mono text-gray-600">
                    <p>b = {coeff.toFixed(4)} · s(b) = {se?.toFixed(4) || "—"}</p>
                    <p>t = b / s(b) = {tStat}</p>
                    <p>Prob &gt; |t| = <span className="text-amber-600 font-semibold">{pFmt}</span> ≥ 0.05 → <span className="text-gray-500">non significatif</span></p>
                  </div>
                </div>

                <div className="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3">
                  <p className="font-semibold text-amber-800 mb-1">Qu'est-ce que ça signifie ?</p>
                ...
                <div className="rounded-lg bg-indigo-50 border border-indigo-200 px-4 py-3">
                  <p className="font-semibold text-indigo-700 mb-1">Que faire ?</p>
                  <ul className="space-y-1 text-indigo-600 leading-relaxed">
                    {dfE < 3 ? (
